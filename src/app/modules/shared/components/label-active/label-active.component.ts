...
export class LabelActiveComponent implements  OnInit, ViewCell {
  ...
  constructor() {}
  ngOnInit(): void {
    if ( this.value === 1 || this.value === true || this.value === 'true' || this.value === 'TRUE' || this.value == 'active') {
      this.label = 'Active';
      this.labelClass = 'badge badge-success mr-1';
    } else if (this.value === 0 || this.value === false || this.value === 'false' || this.value === 'FALSE' || this.value == 'inactive') {
      this.label = 'Inactive';
